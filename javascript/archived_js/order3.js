...
function orderUpdate(form) {
    ...
    pizza_toppings = [];
    custom_toppings = document.getElementsByName("ptoppings");
...
function loadDeliveryForm() {
    ...
    document.getElementById("psize").innerHTML = order_pizza_size;
    document.getElementById("pcrust").innerHTML = order_pizza_crust;
    document.getElementById("ptype").innerHTML = order_pizza_type;   
    document.getElementById("ptoppings").innerHTML = order_pizza_toppings;
...
function validateForm(form) {
    ...
    var important_elements = form.getElementsByClassName("important");
    for(i = 0; i < important_elements.length; i++) {
        important_elements[i].parentNode.removeChild(important_elements[i]);   
    }
    var required_elements = form.getElementsByClassName("required");
    for(i = 0; i < required_elements.length; i++) {
        var parent = required_elements[i].parentElement;
...
//    alert(fname + lname + email+ address+ apartment+ city+ state+ zip + phone);
    var at = 0; //variable to test for @ symbol
    ...
    if(at != 1) {
        ...
        node.appendChild(textnode);
        parent = form.Email.parentElement;
        parent.appendChild(node);
        form.Email.focus();
        return false;   
    } 
    ...
    if(!phone.match(phoneRegEx)) {
        ...
        textnode = document.createTextNode(message);
        node.appendChild(textnode);
        parent = form.Phone.parentElement;
        parent.appendChild(node);
        form.Phone.focus();
        return false;
    }
